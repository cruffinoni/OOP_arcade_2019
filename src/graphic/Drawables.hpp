...
struct Vector3
{
    ...
    Vector3 operator +(Vector3 const &with) const {
        return Vector3(this->x + with.x, this->y + with.y, this->z + with.z);
    }
    Vector3 &operator +=(Vector3 const &with) {
        this->x += with.x; this->y += with.y; this->z += with.z; return *this;
    }
    Vector3 operator -(Vector3 const &with) const {
        return Vector3(this->x - with.x, this->y - with.y, this->z - with.z);
    }
